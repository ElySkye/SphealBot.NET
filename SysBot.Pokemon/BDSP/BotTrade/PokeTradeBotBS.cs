...
namespace SysBot.Pokemon
{
    ...
    public partial class PokeTradeBotBS : PokeRoutineExecutor8BS, ICountBot
    {
        ...
        private async Task<PokeTradeResult> PerformLinkCodeTrade(SAV8BS sav, PokeTradeDetail<PB8> poke, CancellationToken token)
        {
            ...
            await Task.Delay(2_500 + Hub.Config.Timings.ExtraTimeOpenBox, token).ConfigureAwait(false);
            ...
            if (poke.Type == PokeTradeType.Specific)
            {
                if ((GameVersion)toSend.Version == GameVersion.BD || (GameVersion)toSend.Version == GameVersion.SP || toSend.IsEgg)
                {
        ...
        private void UpdateCountsAndExport(PokeTradeDetail<PB8> poke, PB8 received)
        {
        ...
        private async Task<PokeTradeResult> ConfirmAndStartTrading(PokeTradeDetail<PB8> detail, PB8 offered, CancellationToken token)
        {
            ...
            for (int i = 0; i < Hub.Config.Trade.MaxTradeConfirmTime; i++)
            {
                ...
                if (AbuseSettings.AutoBanCooldown)
                {
                    ...
                    if (trash != null && trash.Species != detail.TradeData.Species && trash.Checksum != detail.TradeData.Checksum)
                    {
                        ...
                        {
                            if (detail.Type == PokeTradeType.LinkBDSP)
        ...
        private async Task<bool> EnterUnionRoomWithCode(PokeTradeType tradeType, int tradeCode, CancellationToken token)
        {
            ...
            await Click(A, 0_050, token).ConfigureAwait(false);
            await PressAndHold(A, 5_000, 0, token).ConfigureAwait(false);
            ...
            await PressAndHold(A, 8_500, 0, token).ConfigureAwait(false);
        ...
        private async Task<(PB8 toSend, PokeTradeResult check)> HandleRandomLedy(SAV8BS sav, PokeTradeDetail<PB8> poke, PB8 offered, PB8 toSend, PartnerDataHolder partner, CancellationToken token)
        {
            ...
            if (trade != null)
            {
                ...
                if (tradeevo.Contains(offered.Species) && swap != 229)
                {
            ...
            else if (config.LedyQuitIfNoMatch)
            {
                ...
                var msg = $"**{partner.TrainerName}** has offered **{offers}**\n";
